...
class EncoderDecoderSoft(nn.Module):
    ...
    def forward(self, src, src_mask):
        #print ("src forward encDec", src.size())
        #print ("src_mask forward encDec", src_mask.size())
        out=self.encode(src, src_mask)
        retorno=self.toSoftmax(out)
        return retorno
...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask):
        #print ("x forward encoder", x.size())
        #print ("mask forward encoder", mask.size())
        for layer in self.layers:
            x = layer(x, mask)
            #print ("x forwardeado", x.size())
            
        #print ("x forwardeado final", x.size())
        return self.norm(x)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask):
        #print ("x en encoder layer", x.size())
...
def attention(query, key, value, mask=None, dropout=None):
    ...
    #print ("debiese haber sido", torch.matmul(query, key.transpose(-2, -1)).size())
    if mask is not None:
...
class Batch:
    def __init__(self, src, trg=None, pad=0):
        ...
        #print ("self.src_mask", self.src_mask.size())
